...
#include "llvm/Transforms/Scalar/CFL.h"
...
static cl::opt<bool> SimpleBranches("cfl-simple-branches",
                                    cl::desc("Linearize only simple branches"),
                                    cl::init(false), cl::NotHidden);

static cl::opt<bool> MemProtect(
    "cfl-protect-mem",
    cl::desc("CFL: protect memory accesses (disable if run after DFL)"),
    cl::init(false), cl::NotHidden);

static cl::opt<bool> BranchProtect("cfl-protect-branches",
                                   cl::desc("CFL: protect branches"),
                                   cl::init(true), cl::NotHidden);

static cl::opt<bool>
    CFLRelaxed("cfl-relaxed",
               cl::desc("Avoid linearizing non tainted branches for which we "
                        "may prove they are safe"),
               cl::init(false), cl::NotHidden);


void CFLPass::setInstructionTaint(Instruction *I, bool taint) {
  LLVMContext &C = I->getContext();
  MDNode *N = MDNode::get(C, ConstantAsMetadata::get(ConstantInt::get(
                                 C, APInt(sizeof(imd_t) * 8, taint, true))));
  I->setMetadata("t", N);
}

bool CFLPass::getInstructionTaint(Instruction &I) {
  MDNode *N;
  Constant *val;
  N = I.getMetadata("t");
  assert(N);
...
// loadtainted pass
bool CFLPass::isInstructionUninteresting(Instruction &I) {
  MDNode *N;
...
bool CFLPass::getUninterestingDirection(Instruction &I) {
  MDNode *N;
  Constant *val;
  N = I.getMetadata("uninteresting_direction");
  assert(N);
...
int CFLPass::getBGID(Instruction &I) {
  MDNode *N;
  Constant *val;
...
int CFLPass::getIBID(Instruction &I) {
  MDNode *N;
  Constant *val;
...
ConstantInt *CFLPass::makeConstI32(LLVMContext &C, int value) {
  return ConstantInt::get(C, APInt(sizeof(int) * 8, value, true));
}

ConstantInt *CFLPass::makeConstBool(LLVMContext &C, int value) {
  ConstantInt *BoolTrue = ConstantInt::getTrue(C);
  ConstantInt *BoolFalse = ConstantInt::getFalse(C);
  return value ? BoolTrue : BoolFalse;
}

void CFLPass::wrapIntrinsicCall(CallBase &CS, Function *Callee) {
...
void CFLPass::wrapExtCall(CallBase &CS, Function *Callee) {
...
void CFLPass::wrapLoad(LoadInst *LI) {
  static Function *F =
      LI->getParent()->getParent()->getParent()->getFunction("cfl_ptr_wrap");
...
void CFLPass::wrapStore(StoreInst *SI) {
  static Function *F =
      SI->getParent()->getParent()->getParent()->getFunction("cfl_ptr_wrap");
  static InlineFunctionInfo IFI;
  assert(F);
  std::vector<Value *> args;
  args.push_back(CastInst::CreatePointerCast(
...
void CFLPass::wrapUninterestingCondition(IfCondition &IFC) {
...
void CFLPass::wrapCondition(IfCondition &IFC) {
  ...
  BuilderStart.CreateLifetimeStart(
  ...
  CallInst *MergeCall =
      CallInst::Create(MergePointF, MergePointFArgs, "",
                       &*(IFC.MergePoint->getFirstInsertionPt()));
  ...
  while (PHINode *PN = dyn_cast<PHINode>(IFC.MergePoint->begin())) {
    ...
    Value *TrueVal =
        PN->getIncomingValue(PN->getIncomingBlock(0) == IFC.IfTruePred ? 0 : 1);
    Value *FalseVal =
        PN->getIncomingValue(PN->getIncomingBlock(0) == IFC.IfTruePred ? 1 : 0);
    Value *Sel = SelectInst::Create(IfCond, TrueVal, FalseVal, "",
                                    &*(IFC.MergePoint->getFirstInsertionPt()));
...
BasicBlock *CFLPass::getImmediatePostdominator(PostDominatorTree *PDT, BasicBlock *BB) {
...
IfCondition *CFLPass::getIfCondition(DominatorTree *DT, PostDominatorTree *PDT,
                            BranchInst *BI) {
...
void CFLPass::cfl(Function *F, DominatorTree *DT, PostDominatorTree *PDT) {
...
PreservedAnalyses CFLPass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  //     continue;
  //   if (F.getSection().equals("dfl_code") ||
