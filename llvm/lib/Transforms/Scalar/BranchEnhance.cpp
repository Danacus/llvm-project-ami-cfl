...
#include "llvm/Transforms/Scalar/BranchEnhance.h"

using namespace llvm;
using namespace llvm::PatternMatch;

#define DEBUG_TYPE "branch-enhance"
#define loopenhancePassLog(M)                                                  \
  LLVM_DEBUG(dbgs() << "BranchEnhancePass: " << M << "\n")
...
// an if/else couple is on the set of dependencies. This means that if two
// consecutive branches have the same dependencies we will mark them as a couple
// and destroy the world. So please use only with optimized bc, as the compiler
// will be smart enough to avoid this situation and save ourselves from ethernal
// damnation.

// TAKEN FROM `dump-implicit-deps.cpp` PASS, PLS KEEP THEM COHERENT
// Dump the implicit dependencies of each branch, to counter structurizeCFG
// which introduces them
// -> For each branch which depends on a phi node, gather all the incoming
// values of such phi nodes and add them to the implicit dependencies moreover
// add recursively the conditions of the branches that lead to the phi node
// decision
// NOTICE: this pass does just simple analyses and assumes it runs after
// structurize-cfg

// Given a value, recursively search for all its implicit dependencies
// and save them to the provided set
void searchDependencies(DominatorTree *DT, Value *V,
                        std::set<Instruction *> &dependencies) {
  ...
  if (Instruction *I = dyn_cast<Instruction>(V)) {
    ...
    if (PHINode *Phi = dyn_cast<PHINode>(I)) {
      ...
      for (unsigned int i = 0; i < Phi->getNumIncomingValues(); ++i) {
        ...
        /// \endverbatim
        BranchInst *IncomingTerm =
            dyn_cast<BranchInst>(IncomingBlock->getTerminator());
        assert(IncomingTerm);
...
uint enhanceBranches(Function *F, DominatorTree *DT, PostDominatorTree *PDT) {
  uint NumifElseCouples = 0;
  ...
  for (BasicBlock &BB : *F) {
    ...
    if (!PDT->dominates(MergeBlock, FalseBI->getSuccessor(0)))
      continue;
  ...
  for (auto IfElseBIs : ifElseCouples) {
    ...
    for (PHINode &phi : ElseBI->getParent()->phis()) {
      ...
      for (User *user : phi.users()) {
        ...
        if (DT->dominates(ElseBlock, userPHI->getParent()) &&
            PDT->dominates(ElseEnd, userPHI->getParent())) {
          ...
          // otherwise insert in the set of phi nodes to be moved at the merge
          // point
        } else
          toMove.insert(&phi);
      }
      ...
      for (PHINode *userPHI : elseUsers) {
        userPHI->replaceUsesOfWith(&phi,
                                   phi.getIncomingValueForBlock(IfHeader));
      }
  ...
  // F->dump();
  return NumifElseCouples;
}

PreservedAnalyses BranchEnhancePass::run(Function &F,
                                         FunctionAnalysisManager &AM) {
  uint NumifElseCouples = enhanceBranches(&F, &AM.getResult<DominatorTreeAnalysis>(F),
                  &AM.getResult<PostDominatorTreeAnalysis>(F));
  oprint("IF/ELSE DETECTED: " << NumifElseCouples);
  return PreservedAnalyses::none();
}

// RegisterPass<BranchEnhancePass> MP("branch-enhance", "Branch Enhance
// Pass");